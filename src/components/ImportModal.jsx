      ...
      <div className="modal" style={{ width: 680, maxHeight: '85vh', display: 'flex', flexDirection: 'column' }} onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-body" style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          {!sessions.length && !loading && !fileLoaded && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <p style={{ color: 'var(--text-muted)', fontSize: 13 }}>{t('chooseSource')}</p>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <button className="btn-ghost" onClick={loadPutty}
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: 'var(--text-muted)', fontWeight: 400 }}>{t('puttyDesc')}</div>
                  </div>
                </button>
                <button className="btn-ghost" onClick={loadFile}
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: 'var(--text-muted)', fontWeight: 400 }}>{t('fileDesc')}</div>
                  </div>
                </button>
              </div>
              <div style={{ background: 'var(--bg-base)', borderRadius: 'var(--radius-sm)', padding: '10px 14px', fontSize: 12, color: 'var(--text-muted)', lineHeight: 1.6 }}>
                <strong style={{ color: 'var(--text-secondary)' }}>{t('csvFormat')}</strong> {t('csvColumns')} <code style={{ fontFamily: 'var(--font-mono)', background: 'var(--bg-elevated)', padding: '1px 4px', borderRadius: 3 }}>name,type,host,port,username,notes</code>
              </div>
            </div>
          )}
          ...
          {fileLoaded && !loading && sessions.length === 0 && !error && (
            <div style={{ textAlign: 'center', padding: '32px 16px', color: 'var(--text-muted)' }}>
              ...
              <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 6, color: 'var(--text-secondary)' }}>{t('noServersFound')}</div>
              <div style={{ fontSize: 12, lineHeight: 1.6 }}>
                {t('noServersFoundDesc')} <code style={{ fontFamily: 'var(--font-mono)', background: 'var(--bg-elevated)', padding: '1px 4px', borderRadius: 3 }}>name</code> y <code style={{ fontFamily: 'var(--font-mono)', background: 'var(--bg-elevated)', padding: '1px 4px', borderRadius: 3 }}>host</code> {t('required')}.
              </div>
            </div>
          )}
            ...
            <>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <span style={{ fontSize: 13, color: 'var(--text-secondary)' }}>
                  {sessions.length !== 1 ? t('serversFoundPlural', { n: sessions.length }) : t('serversFound', { n: sessions.length })}
                </span>
                <button className="btn-ghost" onClick={toggleAll} style={{ fontSize: 12 }}>
                  {selected.size === sessions.length ? t('deselectAll') : t('selectAll')}
                </button>
              </div>
        ...
        <div className="modal-footer">
          <button className="btn-ghost" onClick={() => {
            ...
            {sessions.length || fileLoaded ? t('back') : t('cancel')}
          </button>
          {canImport && (
            <button className="btn-primary" onClick={handleImport} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg>
              {selected.size !== 1 ? t('importBtnPlural', { n: selected.size }) : t('importBtn', { n: selected.size })}
            </button>
          )}
        </div>
      </div>
