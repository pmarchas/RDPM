...
export default function SettingsModal({ settings, onSave, onClose }) {
  const [form, setForm] = useState({ configPath: '', homeConfigPath: '', masterPassword: '', viewMode: 'grid', totpSecret: '', lockTimeout: 0, lockOnSystemSleep: true, passwordWarningDays: 90, ...settings });
  ...
  async function confirmTotpSetup() {
    ...
    if (ok) {
      ...
      setTotpConfirmError('Código incorrecto. Comprueba la hora del dispositivo.');
    }
        ...
        <div className="modal-body" style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
          ...
          <div className="settings-section">
            <div className="settings-section-title">
              ...
              Fichero de configuración compartido
            </div>
            <p className="settings-desc">
              Ruta al fichero JSON con todos los servidores. Puede ser local o de red
              (<code>\\servidor\compartido\config.json</code> en Windows, <code>/Volumes/NAS/config.json</code> en macOS).
              Todos los usuarios deben apuntar a la misma ruta.
            </p>
            <div className="field" style={{ marginBottom: 10 }}>
              <label>Ruta del fichero</label>
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <button className="btn-ghost" onClick={browseOpen} style={{ flexShrink: 0, whiteSpace: 'nowrap' }}>Abrir…</button>
                <button className="btn-ghost" onClick={browseCreate} style={{ flexShrink: 0, whiteSpace: 'nowrap' }}>Crear nuevo…</button>
              </div>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              {testResult && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: testResult.ok ? 'var(--success)' : 'var(--danger)' }}>
                  {testResult.ok
                    ? <><svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg>{testResult.exists ? 'Fichero encontrado y legible' : 'Directorio accesible'}</>
          ...
          <div className="settings-section">
            <div className="settings-section-title">
              ...
              Fichero de configuración Home <span style={{ fontSize: 10, fontWeight: 800, letterSpacing: '.06em', color: '#f59e0b', border: '1px solid #f59e0b40', borderRadius: 4, padding: '1px 5px', marginLeft: 4 }}>LOCAL</span>
            </div>
            <p className="settings-desc">
              Ruta al fichero JSON que almacena los servidores del grupo <strong style={{ color: 'var(--text-secondary)' }}>Home</strong>.
              Es local a esta máquina y nunca se comparte con otros usuarios.
              {defaultHomePath && !form.homeConfigPath && (
                <span style={{ display: 'block', marginTop: 6 }}>
                  Ruta por defecto: <code>{defaultHomePath}</code>
                </span>
              )}
            </p>
            <div className="field" style={{ marginBottom: 10 }}>
              <label>Ruta del fichero Home</label>
              <div style={{ display: 'flex', gap: 8 }}>
                <input
                ...
                />
                <button className="btn-ghost" onClick={browseOpenHome}   style={{ flexShrink: 0, whiteSpace: 'nowrap' }}>Abrir…</button>
                <button className="btn-ghost" onClick={browseCreateHome} style={{ flexShrink: 0, whiteSpace: 'nowrap' }}>Crear nuevo…</button>
                {form.homeConfigPath && (
                  <button className="btn-ghost" onClick={clearHomePath} title="Usar ruta por defecto" style={{ flexShrink: 0, color: 'var(--text-muted)' }}>✕</button>
                )}
              </div>
              {!form.homeConfigPath && defaultHomePath && (
                <div className="field-note">Se usará la ruta por defecto: <code style={{ fontFamily: 'var(--font-mono)', fontSize: 11 }}>{defaultHomePath}</code></div>
              )}
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              {homeTestResult && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: homeTestResult.ok ? 'var(--success)' : 'var(--danger)' }}>
                  {homeTestResult.ok
                    ? <><svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg>{homeTestResult.exists ? 'Fichero encontrado y legible' : 'Directorio accesible'}</>
          ...
          <div className="settings-section">
            ...
            <p className="settings-desc">Contraseña usada para cifrar con AES-256 las contraseñas guardadas. Todos los usuarios del mismo fichero deben usar la misma contraseña maestra.</p>
            <div className="field" style={{ marginBottom: 0 }}>
              <label>Contraseña maestra de cifrado</label>
              <div style={{ position: 'relative', display: 'flex' }}>
                <input value={form.masterPassword} onChange={e => set('masterPassword', e.target.value)} type={showPwd ? 'text' : 'password'} placeholder="Contraseña compartida para cifrar/descifrar" autoComplete="new-password" style={{ paddingRight: 42 }} />
              ...
              {!form.masterPassword && <div className="field-note" style={{ color: 'var(--warning)' }}>⚠ Sin contraseña maestra, las contraseñas se guardarán en texto plano</div>}
            </div>
          </div>
          ...
          <div className="settings-section">
            ...
            <p className="settings-desc">
              Protege el acceso a RDPM con un código TOTP de tu app de autenticación (Google Authenticator, Authy, etc.).
            </p>
            ...
            {form.totpSecret && !totpSetup && (
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', background: '#10b98115', border: '1px solid #10b98130', borderRadius: 'var(--radius-sm)', marginBottom: 12 }}>
                  ...
                  <span style={{ fontSize: 13, color: 'var(--success)', fontWeight: 600 }}>2FA activado</span>
                </div>
            ...
            {totpSetup && (
              <div style={{ marginTop: 4 }}>
                <p style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 14, lineHeight: 1.6 }}>
                  Escanea el código QR con tu app de autenticación y luego introduce el código de 6 dígitos para confirmar.
                </p>
                <div style={{ display: 'flex', gap: 20, alignItems: 'flex-start', marginBottom: 16 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 6 }}>Clave manual:</div>
                    ...
                    <div className="field" style={{ marginBottom: 8 }}>
                      <label style={{ fontSize: 12 }}>Código de verificación</label>
                      <input
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button className="btn-primary" onClick={confirmTotpSetup} disabled={totpConfirmCode.length !== 6 || totpConfirming} style={{ fontSize: 13, flex: 1 }}>
                        {totpConfirming ? <span className="spinner" style={{ width: 13, height: 13 }} /> : 'Confirmar'}
                      </button>
                      <button className="btn-ghost" onClick={() => setTotpSetup(null)} style={{ fontSize: 13 }}>Cancelar</button>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="settings-section">
            ...
            <p className="settings-desc">
              Bloquea la pantalla de RDPM por inactividad o al suspender el equipo.
              Las conexiones RDP, VNC y SSH abiertas <strong style={{ color: 'var(--text-secondary)' }}>no se cierran</strong>.
            </p>
            ...
            {!(form.totpSecret || form.masterPassword) && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', background: '#f59e0b12', border: '1px solid #f59e0b30', borderRadius: 'var(--radius-sm)', marginBottom: 12, fontSize: 13, color: '#f59e0b' }}>
                ...
                Configura una contraseña maestra o 2FA para poder activar el bloqueo.
              </div>
            )}

            <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap', alignItems: 'flex-start' }}>
              <div className="field" style={{ marginBottom: 0, flex: 1, minWidth: 160 }}>
                <label>Bloquear tras inactividad</label>
                <select
              ...
              <div style={{ paddingTop: 22 }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', userSelect: 'none' }}>
                  ...
                  <span style={{ fontSize: 13, color: 'var(--text-secondary)' }}>
                    Bloquear al suspender o bloquear la pantalla del sistema
                  </span>
                </label>
              </div>
            </div>
          ...
          <div className="settings-section">
            ...
            <p className="settings-desc">Muestra un aviso en los servidores cuya contraseña no se ha cambiado en X días.</p>
            <div className="field" style={{ marginBottom: 0 }}>
              <label>Avisar tras</label>
          ...
          <div className="settings-section">
            ...
            <div className="field" style={{ marginBottom: 0 }}>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                {[['grid', 'Cuadrícula'], ['list', 'Lista']].map(([val, label]) => (
          ...
          {settingsPath && <div style={{ fontSize: 12, color: 'var(--text-muted)', background: 'var(--bg-base)', borderRadius: 'var(--radius-sm)', padding: '8px 12px', marginTop: 4 }}>Ajustes de esta máquina: <code style={{ fontFamily: 'var(--font-mono)' }}>{settingsPath}</code></div>}
        </div>

        <div className="modal-footer">
          <button className="btn-ghost" onClick={onClose}>Cancelar</button>
          <button className="btn-primary" onClick={() => onSave(form)} disabled={!form.configPath}>Guardar ajustes</button>
        </div>
      </div>
